...
import { ChevronRight, MessageCircle, Mail, X, Download, FileText } from 'lucide-react';
...
const Hero: React.FC = () => {
  ...
  const handleDownloadCv = (lang: 'es' | 'en') => {
    ...
    link.target = '_blank'; // Opcional: abrir en nueva pestaña si la descarga falla
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            ...
            <button
              ...
              className="inline-flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-full text-white bg-slate-800 hover:bg-slate-900 transition-all duration-200 shadow-lg shadow-slate-800/30 cursor-pointer"
            >
              Descargar CV <Download size={20} className="ml-2" />
            </button>
      ...
      {showCvModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-opacity">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 relative animate-in fade-in zoom-in duration-200">
            ...
            <h3 className="text-2xl font-bold text-slate-800 mb-2 text-center">Descargar CV</h3>
            <p className="text-slate-500 text-center mb-6">Selecciona el idioma:</p>

            <div className="space-y-3">
              <button
                onClick={() => handleDownloadCv('es')}
                className="w-full flex items-center justify-center px-4 py-3 bg-slate-800 hover:bg-slate-900 text-white rounded-xl font-medium transition-all shadow-md hover:shadow-lg group"
              >
                <FileText className="mr-3 group-hover:scale-110 transition-transform" size={24} />
                Español
              </button>

              <button
                onClick={() => handleDownloadCv('en')}
                className="w-full flex items-center justify-center px-4 py-3 bg-white border-2 border-slate-100 hover:border-slate-800 hover:text-slate-800 text-slate-700 rounded-xl font-medium transition-all group"
              >
                <FileText className="mr-3 group-hover:scale-110 transition-transform" size={24} />
                Inglés (English)
              </button>
